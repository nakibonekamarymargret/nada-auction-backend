...
const ViewProduct = () => {
  ...
  const [authToken, setAuthToken] = useState(getAuthToken()); // Get the token
  ...
  useEffect(() => {
    ...
    if (product?.auction) {
      ...
      if (targetTime) {
        setTimeRemaining(calculateCountdown(targetTime)); // set immediately

        intervalId = setInterval(() => {
          const newCountdown = calculateCountdown(targetTime);
          setTimeRemaining(newCountdown);

          if (newCountdown.completed) {
            clearInterval(intervalId);

            // Mark product as closed or refetch
            setProduct((prev) => ({
              ...prev,
              auction: {
                ...prev.auction,
    ...
    <div className="container mx-auto px-4 py-6 bg-white shadow-md rounded-lg">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {product.auction?.status !== "CLOSED" && (
          <div className="text-center mx-4 min-w-[160px]">
            <span
              className={`
        ...
        `}
            >
              {timeRemaining.completed
                ? "Auction closed"
                : `Time left: ${timeRemaining.hours}:${timeRemaining.minutes}:${timeRemaining.seconds}`}
            </span>
          </div>
        )}
